...
\r
\r
`,a=new CSSStyleSheet;a.replaceSync(d);class h extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=[a],this.shadowRoot.innerHTML=String(l)}}customElements.define("app-header",h);const p=".card{width:100%;display:flex;flex-direction:column;gap:8px;padding:16px;box-shadow:0 2px 4px #0003;border-radius:12px}.card-title{font-size:1.2rem}.card-sub{color:#666;font-size:.9rem}",f=`<div class="card">\r
...
`,i=new CSSStyleSheet;i.replaceSync(p);class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=[i],this.shadowRoot.innerHTML=String(f)}}customElements.define("app-card",u);
